...
class ShouAndDiTaxiGridGame:
    ...
    def initialize_joint_observation(self, random_grid):
        ########
        # training 시에는 random starting grid, test 시 given starting grid
        ########
    ...
    # row가 (agent_id, matched_demand_id)로 구성된 array return
    def match_agent_and_demand(self, available_agent, temp_joint_observation):
        ...
        for location in range(4):
            # local_demand는 full demand info.가 아닌 local demand row number로 구성된 array
